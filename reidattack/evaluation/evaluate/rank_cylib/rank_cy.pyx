...
# Main interface
cpdef evaluate_reid_cy(distmat, q_pids, g_pids, q_camids, g_camids, max_rank=20, use_metric_cuhk03=False):
...
cpdef eval_market1501_cy(float[:,:] distmat, long[:] q_pids, long[:]g_pids,
    ...
    return np.asarray(avg_cmc).astype(np.float32),\
        np.asarray(all_AP[:valid_index]).mean(axis=0),\
        np.asarray(all_INP[:valid_index]).mean(axis=0)
