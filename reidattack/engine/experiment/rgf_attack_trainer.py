...
class RGFAttackTrainer(NoAttackEvaluator):
    def __init__(
            self,
    ...
    def rgf_attack(self, imgs, camids):
        ...
        adv_imgs = torch.clamp(
            imgs + torch.rand_like(imgs) * 2 * (2 / 255) - (2 / 255),
            0, 1)
        ...
        for _ in range(max_queries // 2):
            ...
            grad = momentum * decay + grad / torch.linalg.vector_norm(
                grad, ord=1, dim=(1, 2, 3), keepdim=True).clamp(min=1e-8)
            momentum = grad
...
def rgf(**trainer_params):
    return RGFAttackTrainer(**trainer_params)
