...
class BanditsUAPAttackEngine(BaseEngine):
    def __init__(
        ...
        self.prior = torch.zeros(1, 3, *self.prior_size, device=self.uap.device)
    ...
    def bandits_uap_attack(self, imgs, camids):
        ...
        for _ in range(max_queries // 2):
            ...
            q1 = F.interpolate(self.prior + exp_noise, size=adv_imgs.shape[-2:])
            ...
            q2 = F.interpolate(self.prior - exp_noise, size=adv_imgs.shape[-2:])
            ...
            grad = F.interpolate(self.prior, size=adv_imgs.shape[-2:])
