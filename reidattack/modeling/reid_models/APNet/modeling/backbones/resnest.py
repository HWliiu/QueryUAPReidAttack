...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.drop_connection_rate > 0:
            out = (
                drop_connect(out, p=self.drop_connection_rate, training=self.training)
                + residual
            )
        else:
            out += residual
    ...
    def __init__(
        ...
        if dilated or dilation == 4:
            self.layer3 = self._make_layer(
                ...
                norm_layer=norm_layer,
                dropblock_prob=dropblock_prob,
                drop_connection_rate=self.global_drop_connect_rate * 0.5,
            )
            self.layer4 = self._make_layer(
                ...
                norm_layer=norm_layer,
                dropblock_prob=dropblock_prob,
                drop_connection_rate=self.global_drop_connect_rate,
            )
        elif dilation == 2:
            self.layer3 = self._make_layer(
                block,
        ...
        for i in range(1, blocks):
            layers.append(
                block(
                    ...
                    dropblock_prob=dropblock_prob,
                    last_gamma=self.last_gamma,
                    drop_connection_rate=drop_connection_rate,
                )
            )
